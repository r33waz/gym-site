...
import type {
  ILoginInterface,
  ISignupInterface,
} from "@/interface/auth.interface"; // Type for login request payload (email + password)
...
const { authLogin, authSignup } = AuthApi; // Destructure the login endpoint config from AuthApi
...
export const useSignup = () => {
  return useMutation({
    mutationFn: (requestData: ISignupInterface) => {
      // Accept login credentials as the mutation argument
      return apiRequest<BackendResponse<null>>({
        // Typed with BackendResponse — no data payload expected on login response
        apiDetails: authSignup, // Pass the login endpoint config (route, method)
        requestData: { ...requestData }, // Spread payload as request body
      });
    },
    onSuccess: (res) => {
      showSuccessMessage({ message: res.message || "Signup successful" }); // Show success toast with backend message or fallback
    },
    onError: (err: any) => {
      showErrorsMessage({ message: err?.message || "Something went wrong" }); // Show error toast with the error message
    },
